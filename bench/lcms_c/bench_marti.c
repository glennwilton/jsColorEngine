 ...
 * pixel count, deterministic image-like input, identical timing loop
...
#define IMAGE_WIDTH        1024 // 256
#define IMAGE_HEIGHT       1024 // 256
...
/* Monotonic clock in nanoseconds using QueryPerformanceCounter. */
static uint64_t now_ns(void)
{
    ...
    /* Split the calculation to avoid overflow from ticks * 1,000,000,000. */
    return (ticks / freq) * UINT64_C(1000000000)
        + ((ticks % freq) * UINT64_C(1000000000)) / freq;
}
...
/* Deterministic 16x16 color blocks model the repeated adjacent pixels
 * commonly found in real images and exercise lcms' one-pixel cache. */
static void build_input(uint8_t* buf, int width, int height, int channels) {
        ...
        for (int x = 0; x < width; x += block_w) {
            ...
            for (int dy = 0; dy < block_h && y + dy < height; dy++) {
                for (int dx = 0; dx < block_w && x + dx < width; dx++) {
                    size_t pixel = ((size_t)(y + dy) * (size_t)width +
                                    (size_t)(x + dx)) * (size_t)channels;
                    for (int c = 0; c < channels; c++)
                        buf[pixel + (size_t)c] = color[c];
                }
            }
        }
    }
 ...
 * The LineStride call is kept here because C has no first-class closures.
 */
static double time_transform(cmsHTRANSFORM xf,
    ...
    cmsUInt32Number in_stride,
    cmsUInt32Number out_stride) {
    /* warmup */
    for (int w = 0; w < WARMUP_ITERS; w++) {
        cmsDoTransformLineStride(xf, in_ptr, out_ptr, width, height,
            in_stride, out_stride, 0, 0);
    }
...
int main(int argc, char** argv) {
    ...
    for (size_t w = 0; w < N_WORKFLOWS; w++) {
        ...
        build_input(in_buf, IMAGE_WIDTH, IMAGE_HEIGHT, wf->in_ch);
        ...
        double ms_def = time_transform(xfDef, in_buf, out_buf,
        ...
        double ms_hi = time_transform(xfHi, in_buf, out_hi,
            IMAGE_WIDTH, IMAGE_HEIGHT,
            IMAGE_WIDTH * wf->in_ch, IMAGE_WIDTH * wf->out_ch);
