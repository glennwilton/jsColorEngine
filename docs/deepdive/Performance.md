...
> **This page is historical.** Figures are performance at the time of
> writing. Re-run on your machine: browser
> [`samples/bench/`](../../samples/bench/) (live:
> https://www.o2creative.co.nz/jscolorengine/samples/bench/) or Node
> `node bench/mpx_summary.js`. Quote current throughput from
> **[Bench.md](../Bench.md)** and **[BenchResults.md](../BenchResults.md)**.
> LittleCMS comparison: **[LcmsComparison.md](../LcmsComparison.md)**.
>
...
> [deepdive/benchmark.md § 20b](./benchmark.md#20b-two-more-ways-the-input-lied-2026-08-19).
>
> ### The figures here are real, but narrow
>
> Read **[benchmark.md § 21 — Noise is the great equaliser](./benchmark.md#21-noise-is-the-great-equaliser)**
...
> **Current numbers live in [Bench.md](../Bench.md) and
> [BenchResults.md](../BenchResults.md).** This page is the
> measurement retrospective — the journey, the dead ends, the
> lessons. Sibling pages in this folder have the V8 asm, `.wat`
> design, and reproduction recipes.
...
[deep-dive / WASM kernels](./WasmKernels.md)). All four
...
design, are documented in [deep-dive / WASM kernels](./WasmKernels.md):

- [WASM scalar — 3D](./WasmKernels.md#wasm-scalar-3d--the-first-kernel)
  (avg **1.40×** over `'int'`, range 1.37-1.45×)
- [WASM SIMD — 3D](./WasmKernels.md#wasm-simd-3d--channel-parallel-was-the-wrong-axis-once)
  (avg **3.25×** over `'int'`, range 2.94-3.50×)
- [WASM scalar — 4D](./WasmKernels.md#wasm-scalar-4d--and-the-function-call-lesson)
  (avg **1.22×** over `'int'`, range 1.13-1.49×)
- [WASM SIMD — 4D](./WasmKernels.md#wasm-simd-4d--the-k-plane-loop-in-one-v128)
...
[deep-dive / WASM kernels — SIMD 3D](./WasmKernels.md#wasm-simd-3d--channel-parallel-was-the-wrong-axis-once).)
...
**→ See [deep-dive / JIT inspection](./JitInspection.md)** for
...
**→ See [deep-dive / JIT inspection — "Does 'named temps hurt'?"](./JitInspection.md#does-named-temps-hurt--a-micro-test-that-is-not-the-final-word)**
...
**→ See [deep-dive / WASM kernels — scalar 3D](./WasmKernels.md#wasm-scalar-3d--the-first-kernel)**
for the 6-config matrix, alpha-mode handling, and the
dispatch-counter test pattern.
**→ See [deep-dive / WASM kernels — scalar 4D](./WasmKernels.md#wasm-scalar-4d--and-the-function-call-lesson)**
...
**→ See [deep-dive / WASM kernels — SIMD 3D](./WasmKernels.md#wasm-simd-3d--channel-parallel-was-the-wrong-axis-once)**
...
**→ See [deep-dive / WASM kernels — SIMD 4D](./WasmKernels.md#wasm-simd-4d--the-k-plane-loop-in-one-v128)**
...
**What we did.** [Deep-dive / JIT inspection](./JitInspection.md#implications-for-future-work)
...
— see [JitInspection.md](./JitInspection.md#implications-for-future-work))
are now provably most worth running against the **x86 baseline**, since
the ARM CPUs already extract that headroom for free.

**→ See [deep-dive / JIT inspection — implications for future work](./JitInspection.md#implications-for-future-work)**
...
**→ See [deep-dive / WASM kernels — function-call cost lesson](./WasmKernels.md#the-function-call-cost-lesson-v8-wasm-inliner-was-the-hidden-tax).**
...
summarised in §2.1 (full tables in [deep-dive / JIT inspection](./JitInspection.md)):
...
| **Register pressure** | wasm32 is a register-allocated VM, not true hardware regs | V8 compiles to real x64/ARM64 registers directly; 4D kernels do spill (see [deep-dive / JIT inspection](./JitInspection.md)) but stay L1i-resident |
...
| `src/wasm/tetra4d_nch.wat` — 4D scalar (CMYK input), cMax ∈ {3, 4, 5+}, same 4-mode alpha, hoisted-prologue + flag-gated K-plane loop, 1.22× over JS `int` (see [deep-dive / WASM kernels — scalar 4D](./WasmKernels.md#wasm-scalar-4d--and-the-function-call-lesson)) | ✅ shipped |
| `lutMode: 'int-wasm-scalar'` 4D dispatcher for inputChannels=4 + Jest suite + 4-mode alpha coverage | ✅ shipped |
| `src/wasm/tetra4d_simd.wat` — 4D channel-parallel SIMD kernel, cMax ∈ {3, 4}, hoisted-prologue + flag-gated K-plane loop + u20 K-LERP, K0 in v128 register (no scratch), 2.39× over JS `int` / 1.98× over WASM scalar 4D (see [deep-dive / WASM kernels — SIMD 4D](./WasmKernels.md#wasm-simd-4d--the-k-plane-loop-in-one-v128)) | ✅ shipped |
...
[deep-dive / WASM kernels — SIMD 4D](./WasmKernels.md#wasm-simd-4d--the-k-plane-loop-in-one-v128)):
...
- ~~4D scalar `.wat` port.~~ **Shipped** — measured 1.22× avg
  ...
  configs. See [deep-dive / WASM kernels — scalar 4D](./WasmKernels.md#wasm-scalar-4d--and-the-function-call-lesson)
...
- ~~4D SIMD `.wat` port.~~ **Shipped** — measured 2.39× avg over
  ...
  / WASM kernels — SIMD 4D](./WasmKernels.md#wasm-simd-4d--the-k-plane-loop-in-one-v128)
...
- Extend the 6-config 3D bench to a 12-config matrix that includes
  ...
  [deep-dive / WASM kernels](./WasmKernels.md).
...
1. **Q0.13 weights, not Q0.16.** The standard fixed-point precision
   ...
   [accuracy bench](./Accuracy.md#16-bit-kernel-accuracy-v13--near-perfect-no-corners-cut)
...
- [`bench/int16_poc/accuracy_v1_7_self.js`](../../bench/int16_poc/accuracy_v1_7_self.js)
  ...
  [Accuracy.md § 16-bit kernel accuracy](./Accuracy.md#16-bit-kernel-accuracy-v13--near-perfect-no-corners-cut).
...
see [deepdive/KernelContract.md](./KernelContract.md) — Node
...
2. **WASM SIMD with across-pixel LUT gather is *slower* than WASM
   ...
   SIMD 3D](./WasmKernels.md#wasm-simd-3d--channel-parallel-was-the-wrong-axis-once)
