...
| [Synthetic profiles](./SyntheticProfiles.md)     | Three of five kernels had no oracle, because real ICC profiles are licensed and this repo ships two. So the engine writes its own: `encodeICC.js`, gray and 2CLR-10CLR, handed to Little CMS. Kernel2D agrees bit for bit; gray is 100% within 1 LSB. Records the three bugs it found on its first runs, the noise-versus-smooth mistake that made a working engine look catastrophic, and why the interpolator changed. |
