...
| [Synthetic profiles](./SyntheticProfiles.md)     | Three of five kernels had no oracle, because real ICC profiles are licensed and this repo ships two. So the engine writes its own: `encodeICC.js`, fifteen dual-table profiles at 1-15 channels, handed to Little CMS. Every input width converts into every output width, both depths. Kernel2D agrees bit for bit. Records the five bugs it found -- including 165 of 225 conversions broken by a stage name built with string concatenation -- and the noise-versus-smooth mistake that made a working engine look catastrophic. |
