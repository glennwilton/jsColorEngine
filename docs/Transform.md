...
| `pixelCache` | Number | `0` | Memoise the accuracy path: `0`/`false` off, `1` a single entry (catches solid fills and runs), `16`/`32`/… a direct-mapped table (also catches repeating palettes and dithers; rounded down to a power of two). Opt-in and off by default — it is a win on flat and synthetic content and a small tax on photographic content, so measure before enabling. No effect on the LUT image path. Read `getPixelCacheStats()` for `{hits, misses, lookups, hitRate}`. Declines silently (leaving a correct, uncached pipeline) when `pipelineDebug` is on or custom stages are present. See [deepdive/PixelCache.md](./deepdive/PixelCache.md). |
