...
| **`lcms HIGHRES`** | Same, with `cmsFLAGS_HIGHRESPRECALC`. **Diagnostic only** — per upstream ([#6](https://github.com/glennwilton/jsColorEngine/issues/6)) this is a legacy lcms 1.x emulation flag, not an accuracy upgrade; `lcms default` is the representative number (the two measure within ±1.5 %) |
