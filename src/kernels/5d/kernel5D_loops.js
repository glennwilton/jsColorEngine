// src/kernels/5d/kernel5D_loops.js
//
// 5-channel input array loops. Float delegates to tetrahedralInterpND_NCh
// (same Little CMS peel). Int8 is the WASM oracle: two u20 peels (E then K)
// around the 6-case tetra on the last three axes.
'use strict';
...
function tetrahedralInterp5DArray_NCh_loop(input, inputPos, output, outputPos, length, lut, inputHasAlpha, outputHasAlpha, preserveAlpha){
    var inCh = 5;
...
function tetrahedralInterp5DArray_NCh_intLut_loop(input, inputPos, output, outputPos, length, intLut, inputHasAlpha, outputHasAlpha, preserveAlpha){
    ...
    for(p = 0; p < length; p++){
        ...
        for(epass = 0; epass < eLim; epass++){
            for(kpass = 0; kpass < kLim; kpass++){
                plane = (E.lo + (epass ? go4 : 0) + K.lo + (kpass ? go3 : 0)) | 0;
                ...
                for(o = 0; o < cMax; o++){
                    ...
                    output[outputPos++] = epass === 1
                        ? tetra.emitU8Lerp(scratchE[o], v, E.r)
                        : tetra.emitU8(v);
                }
            }
        }
...
module.exports = {
    tetrahedralInterp5DArray_NCh_loop: tetrahedralInterp5DArray_NCh_loop,
    tetrahedralInterp5DArray_NCh_intLut_loop: tetrahedralInterp5DArray_NCh_intLut_loop,
};
